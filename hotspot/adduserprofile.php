...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (isset($_POST['name'])) {
    ...
    if ($expmode == "rem") {
      $expiryAction = $recordOnExpire . '[ /ip hotspot user ' . $mode . ' $i ]; [ /ip hotspot active remove [find where user=$name] ];';
    } elseif ($expmode == "ntf") {
      $expiryAction = ':local limituptime [ /ip hotspot user get $i limit-uptime ]; :if ($limituptime != "1s") do={' . $recordOnExpire . '[ /ip hotspot user ' . $mode . ' $i ]; [ /ip hotspot active remove [find where user=$name] ];};';
    } elseif ($expmode == "remc") {
      $expiryAction = '[ /ip hotspot user ' . $mode . ' $i ]; [ /ip hotspot active remove [find where user=$name] ];';
    } elseif ($expmode == "ntfc") {
      $expiryAction = ':local limituptime [ /ip hotspot user get $i limit-uptime ]; :if ($limituptime != "1s") do={[ /ip hotspot user ' . $mode . ' $i ]; [ /ip hotspot active remove [find where user=$name] ];};';
    } else {
      $expiryAction = '';
    }

    $bgservice = ':local dateint do={:local montharray ( "jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec" );:local days [ :pick $d 4 6 ];:local month [ :pick $d 0 3 ];:local year [ :pick $d 7 11 ];:local monthint ([ :find $montharray $month]);:local month ($monthint + 1);:if ( [len $month] = 1) do={:local zero ("0");:return [:tonum ("$year$zero$month$days")];} else={:return [:tonum ("$year$month$days")];}}; :local isoDateInt do={ :local year [ :pick $d 0 4 ]; :local month [ :pick $d 5 7 ]; :local days [ :pick $d 8 10 ]; :return [:tonum ("$year$month$days")]; }; :local timeint do={ :local hours [ :pick $t 0 2 ]; :local minutes [ :pick $t 3 5 ]; :return ($hours * 60 + $minutes) ; }; :local date [ /system clock get date ]; :local time [ /system clock get time ]; :local today [$dateint d=$date] ; :local curtime [$timeint t=$time] ; :foreach i in [ /ip hotspot user find where profile="'.$name.'" ] do={ :local comment [ /ip hotspot user get $i comment]; :local name [ /ip hotspot user get $i name]; :local expd 0; :local expt 0; :if (([:len $comment] >= 19) and ([:pick $comment 4 5] = "-") and ([:pick $comment 7 8] = "-") and ([:pick $comment 10 11] = " ")) do={ :local getdate [:pick $comment 0 10]; :local gettime [:pick $comment 11 19]; :set expd [$isoDateInt d=$getdate]; :set expt [$timeint t=$gettime]; } else={ :if (([:len $comment] >= 20) and ([:pick $comment 3 4] = "/") and ([:pick $comment 6 7] = "/")) do={ :local getdate [:pick $comment 0 11]; :local gettime [:pick $comment 12 20]; :set expd [$dateint d=$getdate]; :set expt [$timeint t=$gettime]; }}; :if ($expd > 0) do={ :if (($expd < $today and $expt < $curtime) or ($expd < $today and $expt > $curtime) or ($expd = $today and $expt < $curtime)) do={ ' . $expiryAction . '}}}';
